module internal PgSqlManaged.ext
...
type NpgsqlCommand with
    ...
    /// 创建一个 PgSqlTransaction, 并以其为参数执行闭包 f
    /// PgSqlTransaction 销毁权交由闭包 f
    ...
    /// 托管一个 PgSqlTransaction, 并以其为参数执行闭包 f
    /// 闭包执行完成后该 PgSqlTransaction 会被销毁
...
type NpgsqlConnection with
    ...
    /// 创建一个 PgSqlCommand, 并以其为参数执行闭包 f
    /// PgSqlCommand 销毁权交由闭包 f
    member self.useCommand f =
        let cmd = self.CreateCommand()
        f cmd

    /// 托管一个 PgSqlCommand, 并以其为参数执行闭包 f
    /// 闭包执行完成后该 PgSqlCommand 会被销毁
